...
twitch_miner = TwitchChannelPointsMiner(
    username="rocky1567",
    password="plokijuhygtfrdeswaplokijuhygtfrdeswaq",           # If no password will be provided, the script will ask interactively
    ...
    logger_settings=LoggerSettings(
        ...
        discord=Discord(
            webhook_api="https://discord.com/api/webhooks/1241429422108573746/QI10b6YYB24lAzWN0oHLoXZku4Vy4KCUB1ks0vznZaK_EgdJGagevb9l-muymt6Qb5Z9",  # Discord Webhook URL
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.GAIN_FOR_CLAIM, Events.GAIN_FOR_WATCH, Events.CHAT_MENTION],                                  # Only these events will be sent to the chat
        ),
        matrix=Matrix(
...
twitch_miner.mine(
    [
       ...
       Streamer("Apollo_Protocol", settings=StreamerSettings(make_predictions=True , follow_raid=True  , claim_drops=True ,                     bet=BetSettings(strategy=Strategy.PERCENTAGE , percentage=5 , stealth_mode=False, percentage_gap=20 , max_points=1234  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
       Streamer("bojkasi", settings=StreamerSettings(make_predictions=True , follow_raid=True  , claim_drops=True ,                     bet=BetSettings(strategy=Strategy.PERCENTAGE , percentage=5 , stealth_mode=False, percentage_gap=20 , max_points=1234  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
